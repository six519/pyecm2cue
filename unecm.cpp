...
initpyecm2cue(void){
    ...
    m = Py_InitModule("pyecm2cue", PyECM2CueMethods);
    if (m == NULL)
        return;

    PyECM2CueError = PyErr_NewException("pyecm2cue.Error", NULL, NULL);
    Py_INCREF(PyECM2CueError);
    PyModule_AddObject(m, "Error", PyECM2CueError);
